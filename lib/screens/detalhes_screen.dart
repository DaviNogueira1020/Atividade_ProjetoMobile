...
class _DetalhesScreenState extends State<DetalhesScreen> {
  ...
  void _atualizarObservacoes() {
    ...
    final chamadoAtualizado = _chamado.copyWith(
      observacoes: observacoes,
    );

    context.read<ChamadoProvider>().atualizarChamado(chamadoAtualizado).then((_) {
      final provider = context.read<ChamadoProvider>();
      _chamado = provider.chamados.firstWhere(
        (c) => c.id == chamadoAtualizado.id,
        orElse: () => _chamado,
      );
      setState(() {
        _editando = false;
        ...
        child: Column(
          ...
          children: [
            ...
            if (!_chamado.isConcluido)
              Column(
                children: [
                  if (_chamado.status != StatusChamado.emProgresso)
                    SizedBox(
                      width: double.infinity,
                      child: ElevatedButton.icon(
                        icon: const Icon(Icons.play_arrow),
                        label: const Text('Iniciar'),
                        onPressed: () => _alterarStatus(StatusChamado.emProgresso),
                      ),
                    ),
                  const SizedBox(height: 12),
                  SizedBox(
